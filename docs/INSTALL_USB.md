...
- Die Dateien `clausis-0.1.2-amd64.iso` und
  `clausis-0.1.2-amd64.iso.sha256` aus `dist/`.
...
Auf macOS kann ersatzweise `shasum -a 256 clausis-0.1.2-amd64.iso` verwendet
...
2. „Eigenes Image“ und `clausis-0.1.2-amd64.iso` auswählen.
...
Diese 0.1.2-Version ist ein technischer Teststand. Vor dem Einsatz müssen
