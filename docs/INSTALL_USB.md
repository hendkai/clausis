...
- Die Dateien `clausis-0.2.0-amd64.iso` und
  `clausis-0.2.0-amd64.iso.sha256` aus `dist/`.
...
`shasum -a 256 -c clausis-0.2.0-amd64.iso.sha256` verwendet.

## Empfohlener grafischer Weg

1. Raspberry Pi Imager oder balenaEtcher öffnen.
2. „Eigenes Image“ und `clausis-0.2.0-amd64.iso` auswählen.
...
Diese 0.2.0-Version ist ein technischer Teststand. Vor dem Einsatz müssen
