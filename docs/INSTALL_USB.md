# VoiceOS-Installationsstick erstellen
...
- Die Dateien `voiceos-0.1.0-amd64.iso` und
  `voiceos-0.1.0-amd64.iso.sha256` aus `dist/`.
...
Auf macOS kann ersatzweise `shasum -a 256 voiceos-0.1.0-amd64.iso` verwendet
...
2. „Eigenes Image“ und `voiceos-0.1.0-amd64.iso` auswählen.
...
VoiceOS startet als Live-System. Nach dem GNOME-Start wird der KI-Hinweis
gesprochen und der Calamares-Installer geöffnet. Orca kann jederzeit mit
`Super`+`Alt`+`S` aktiviert werden. Die Anwendung „VoiceOS Sprachsteuerung“
...
Diese 0.1.0-Version ist ein technischer Teststand. Vor dem Einsatz müssen
