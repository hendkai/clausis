...
docker run --rm --platform linux/amd64 --entrypoint sh \
    -v "$project_dir/dist:/artifacts:ro" "$builder" -ec '
        xorriso -indev /artifacts/clausis-0.3.1-amd64.iso -check_media -- >/tmp/media-check 2>&1
        xorriso -indev /artifacts/clausis-0.3.1-amd64.iso -report_el_torito plain >/tmp/boot-report 2>&1
        xorriso -indev /artifacts/clausis-0.3.1-amd64.iso -find / -type f -exec echo -- >/tmp/file-list 2>&1
