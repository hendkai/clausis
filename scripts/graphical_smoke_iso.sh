...
iso="$project_dir/dist/clausis-0.3.0-amd64.iso"
image="clausis-boot-smoke:0.3.0"
screenshot="clausis-0.3.0-boot-screen.png"
...
docker run --rm --entrypoint sh -v "$project_dir/dist:/artifacts" "$image" -ec '
    mkdir -p /tmp/boot
    : > /artifacts/clausis-0.3.0-graphical-boot.log
    : > /artifacts/clausis-0.3.0-graphical-qemu.log
    : > /artifacts/clausis-0.3.0-monitor.log
    xorriso -osirrox on -indev /artifacts/clausis-0.3.0-amd64.iso \
    ...
    qemu-system-x86_64 \
        ...
        -drive file=/artifacts/clausis-0.3.0-amd64.iso,media=cdrom,readonly=on \
        -display none -vga std \
        -serial file:/artifacts/clausis-0.3.0-graphical-boot.log \
        -monitor unix:/tmp/qemu-monitor,server=on,wait=off \
        -no-reboot >/artifacts/clausis-0.3.0-graphical-qemu.log 2>&1 &
    qemu_pid=$!
    cleanup() {
    ...
    while [ "$elapsed" -lt 360 ] && kill -0 "$qemu_pid" >/dev/null 2>&1; do
        if grep -Eq "Started .*GNOME Display Manager|Started .*gdm.service" /artifacts/clausis-0.3.0-graphical-boot.log 2>/dev/null; then
            ready=1
            break
        fi
        sleep 2
    ...
    if [ "$ready" -ne 1 ]; then
        tail -120 /artifacts/clausis-0.3.0-graphical-boot.log 2>/dev/null || true
        exit 1
    fi
    ...
    while [ "$elapsed" -lt 300 ] && kill -0 "$qemu_pid" >/dev/null 2>&1; do
        ...
        printf "screendump /tmp/boot-screen.ppm\n" \
            | socat - UNIX-CONNECT:/tmp/qemu-monitor \
                >/artifacts/clausis-0.3.0-monitor.log 2>&1
        ...
        if [ "$png_bytes" -ge 20000 ]; then
            ...
            pnmtopng /tmp/boot-screen.ppm \
                > /artifacts/clausis-0.3.0-boot-screen.png
            final_png_bytes=$(wc -c < /artifacts/clausis-0.3.0-boot-screen.png)
    ...
    grep -E "Started .*GNOME Display Manager|Started .*gdm.service" /artifacts/clausis-0.3.0-graphical-boot.log | tail -1
  '
